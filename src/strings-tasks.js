...
function removeFirstOccurrences(str, value) {
  ...
  if (startIndex !== -1) {
    ...
    const lastPart = str.slice(startIndex + value.length);
    newString = firstPart + lastPart;
  }
  return newString;
}
...
function isPalindrome(str) {
  const cleanStr = str.replace(/[^\w]/gi, '').toLowerCase();
  const reverseStr = cleanStr.split('').reverse().join('');
  return cleanStr.toLowerCase() === reverseStr;
}
...
function reverseWords(str) {
  const arr = str.split(' ');
  for (let i = 0; i < arr.length; i += 1) {
    const reverseWord = arr[i].split('').reverse().join('');
    arr[i] = reverseWord;
  }
  return arr.join(' ');
}
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    for (let j = 0; j < arr.length; j += 1) {
      if (/[a-zA-Z]/.test(arr[j])) {
        const index = orig.indexOf(arr[j]);
        newStr += cpArr[index];
      } else {
        newStr += arr[j];
      }
